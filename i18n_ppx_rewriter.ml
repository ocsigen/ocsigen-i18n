...
let mk_ident i =
  ...
  let module_name, safe_ident =
    ...
    | i ->
        let err_msg =
          Format.asprintf "%s is not a valid i18n expression" (Longident.name i)
        in
        failwith err_msg
  in
  let rec prefix i =
    match i with
    | Lident id -> Ldot (Ldot (Lident module_name, "Tr"), id)
    | Ldot (i, nm) -> Ldot (prefix i, nm)
    | Lapply _ ->
        let err_msg =
          Format.asprintf "%s is not a valid i18n expression" (Longident.name i)
        in
...
let () =
  List.iter
    ...
    [ ( "--prefix"
      , Arg.Set_string module_prefix
      , "PREFIX The prefix added to module names" )
    ; ( "--suffix"
      , Arg.Set_string module_suffix
      , "SUFFIX The suffix added to module names" )
    ; ( "--default-module"
      , Arg.Set_string default_module_name
...
let () = Ppxlib.Driver.register_transformation ~rules:[rule] "i18n"
